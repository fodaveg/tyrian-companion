...
describe('Halloween price alert runtime', () => {
	...
	it('emits only a durable below-to-high crossing, at most once per day, and requires a valid below to rearm', async () => {
		...
		await runtime.evaluate(port(20), NOW + 1);
		await runtime.evaluate(port(40), NOW + 2);
		...
		await runtime.evaluate(port(40), NOW + 3);
		await runtime.evaluate({ readDaily: async () => [] }, NOW + 4);
		await runtime.evaluate(port(40), NOW + 5);
		expect(onNotice).toHaveBeenCalledOnce();
		runtime.dispose();
	});
	...
	it('keeps daily suppression across settings changes and competing windows', async () => {
		...
		await Promise.all([first.evaluate(port(40), NOW + 1), second.evaluate(port(40), NOW + 1)]);
	...
	it('drops a stale local read after dispose and fails closed for future or corrupt H11 state', async () => {
		...
		tx.objectStore(HALLOWEEN_PRICE_ALERT_STORE).put({ version: 99, vaultId: 'vault', accountRef: 'account', itemId: 36_038 });
...
function port(today: number) { return { readDaily: async () => history(today) }; }

function history(today: number): PriceHistoryDailyV1[] {
	return [...Array.from({ length: 30 }, (_, index) => daily(NOW - (30 - index) * 86_400_000, index + 1)), daily(NOW, today)];
}
