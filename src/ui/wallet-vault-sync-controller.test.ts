...
import {
	...
	summarizeWalletVaultSyncPlan,
	type WalletVaultSyncControllerPorts,
} from './wallet-vault-sync-controller';

describe('wallet Vault sync controller', () => {
	...
	it('enters loading only after explicit preview and exposes a closed summary', async () => {
		const pending = deferred<WalletVaultSyncPlan>();
	...
	it('requires a successful explicit preview before apply and projects applying then success', async () => {
		const applying = deferred<WalletVaultSyncResult>();
	...
	it('summarizes only the closed plan enums', () => {
		expect(summarizeWalletVaultSyncPlan(planWith([
...
function portsFor(overrides: Partial<WalletVaultSyncControllerPorts> = {}): WalletVaultSyncControllerPorts & {
...
function planWith(
	statuses: WalletVaultSyncPlan['steps'][number]['status'][],
