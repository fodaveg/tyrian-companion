...
describe('buildCompanionStatus', () => {
	...
	it.each([
		...
		['account_changed', 'Final: La clave de API ahora es de otra cuenta, no de la que empezó la sesión. La sesión se conserva y no se reintenta sola: vuelve a poner la clave de esa cuenta y termina otra vez.', 'Stop: The API key now belongs to another account, not the one this session started with. The session is kept and will not retry on its own: put that account\'s key back and finish again.'],
