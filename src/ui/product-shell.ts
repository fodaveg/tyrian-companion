...
export function mountActionPanel(controller: ProductActionController, locale: Locale): ProductActionPanelMount {
	...
	for (const group of ['navigation', 'session', 'detection', 'inventory'] as const) {
		...
		panel.append(renderGroup(group, actions, controller, copy, actionNodes));
	}
	const feedback = panel.createDiv({ cls: 'tyrian-action-panel__feedback' });
	feedback.setAttr('role', 'status');
	feedback.setAttr('aria-live', 'polite');
	panel.createEl('p', { text: copy.palette, cls: 'tyrian-action-panel__palette-note' });
	const update = (): void => {
		...
		feedback.setText(current === null ? '' : `${controller.describe(current.actionId).name}: ${current.message}`);
	...
	update();
	return { element: panel, update, dispose: unsubscribe };
}
