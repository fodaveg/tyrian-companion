...
describe('inventory discard allowlist H4.16', () => {
	...
	it.each([
		...
		['applicable knowledge', (value: InventoryAdvisorEngineInputV1) => { value.knowledgePack.entries[0]!.use = { status: 'applicable', ruleId: 'use-10', sourceIds: ['knowledge-source'] }; (value.input.rulePack as InventoryAdvisorRulePackV1).rules.unshift(rule('use-10', 'use', 'curated_use')); }],
		['duplicate discard rule', (value: InventoryAdvisorEngineInputV1) => { (value.input.rulePack as InventoryAdvisorRulePackV1).rules.push(rule('discard-11', 'discard_candidate', 'curated_discard_review')); }],
...
function fixture(): InventoryAdvisorEngineInputV1 {
	...
	const rulePack: InventoryAdvisorRulePackV1 = { schemaVersion: 1, id: 'rules', version: 1, publishedAt: '2026-08-01T00:00:00.000Z', reviewedAt: '2026-08-02T00:00:00.000Z', validUntil: '2027-01-01T00:00:00.000Z', sha256: '', sources: [{ id: 'rule-source', url: 'https://wiki.guildwars2.com', retrievedAt: '2026-08-02T00:00:00.000Z' }], rules: [rule('discard-10', 'discard_candidate', 'curated_discard_review')] };
...
function rule(ruleId: string, action: InventoryAdvisorRuleV1['action'], reason: InventoryAdvisorRuleV1['reason']): InventoryAdvisorRuleV1 { return { ruleId, itemId: 10, action, status: 'approved', assertion: 'applicable', reason, sourceIds: ['rule-source'] }; }
