...
- H4.13 fija el contrato defensivo del Inventory Advisor para `supported_storage_v1`, no para toda la cuenta. Catálogo, precios, objetivos, excepciones de conservación, desbloqueos y reglas revisadas quedan ligados al mismo snapshot. La salida particiona cantidades y solo describe acciones manuales; `discard_candidate` es una revisión irreversible curada y nunca una orden de destruir. H4.14-H4.16 cubren captura, motor y allowlist pura. H4.18 aporta un bundle built-in v2 source-backed para el item 36038: `open` es una capacidad demostrada, `use` excluye abrir y `salvage` no aplica; una compuerta de recomendación separada mantiene la comparación económica en revisión. Por ello no se habilita `open`, economía ni descarte. H5.11 añade la vista manual separada: abrir no captura y actualizar compone esas capas una sola vez. H5.12 permite editar localmente objetivos y excepciones sólo después de captura, con CAS y reclasificación fresca sin operar en GW2. Al expirar el bundle, la actualización bloquea antes de red con `missing_rules`.
- H4.14 ya captura manualmente la evidencia completa de esa superficie: catálogo para cada item propio,
  ...
  H5.12 ya aporta edición local explícita con CAS. H4.18 ya conoce la capability curada de abrir el item
  36038, pero permanece retenida por revisión humana pendiente y comparación económica ausente: no hay
  ninguna recomendación activa.
