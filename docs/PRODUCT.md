...
- «Abandonar sesión» (David aprobó el 24 sep 2026) resuelve el `stopping` que ningún reintento puede terminar: solo aparece con los fallos `account_changed` y `delta_invalid`, donde el baseline y el final ya no son comparables. Pide confirmación explícita (mantener la sesión es el foco por defecto) y no ofrece salida ante fallos de red, rate limit, lease o coordinación, que siguen reintentando hasta un resultado real. Al confirmar, la sesión termina `abandoned` con motivo y sin snapshot final, delta, precio ni botín; libera el lease y el runtime guardado, y detección/marcado automático se rearman solos. La nota usa el esquema 6 (`tc_outcome`, `tc_abandon_reason`), conserva cada línea humana y H18.10 cuenta las sesiones abandonadas aparte, fuera de totales y comparación de rendimiento.
...
- Panel remoto o sincronizado de precisión/recall. H7.13 calcula y exporta bajo acción explícita las métricas locales del piloto, pero no sincroniza telemetría ni mezcla plataformas.
