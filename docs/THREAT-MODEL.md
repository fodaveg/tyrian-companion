...
| Falso positivo económico o de skin/mini | Rare sin precio exige `no_quote` demostrado; 429/offline/payload inválido no califican. Unlock solo se emite con catálogo y cobertura completa simultánea de skins/minis; permiso ausente, parcial o error fallan cerrados. Una skin aplicada no se confunde con `details.skins[]`. | ArenaNet puede cambiar disponibilidad o semántica; la señal siempre exige revisión manual. |
| Contaminación o duplicación de alertas | Solo una sesión Halloween aceptada habilita vivo. Polling y final comparten `session:<id>`; el final sustituye atómicamente la evidencia provisional y no emite otro Notice. Seen, episode y meta validan scope/version/payload. | Episodios distintos pueden alertar de nuevo deliberadamente. Un Vault ilegible detiene el aprendizaje en vez de inventar cobertura. |
