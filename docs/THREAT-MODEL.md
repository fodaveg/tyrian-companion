...
| Contrato H8.4 IPC local | Esquemas, framing, token/nonce, secuencia, heartbeat, plazos y errores declarativos | No genera secretos, abre sockets ni procesa records. Los valores sintéticos, validators, framer y fake clock viven solo en tests. |
...
| Desincronización o DoS del canal H8.4 | Heartbeat y sample comparten secuencia exacta `0,+1`; gap/replay/regresión/overflow/wrap cierran. Discovery vence en 5 s y connect/hello/primer record/salud en 2 s. Fallo pre-ready exige restart; solo post-ready permite reconnect al mismo helper. `helper_exited` en cualquier estado no terminal, incluido reconnect, invalida token/puerto/nonce/secuencia y bloquea `reconnect_due`. La tabla de fases rechaza records fuera de fase; heartbeat/sample válidos renuevan salud y solo `healthy` resetea backoff. EOF invalida/apaga y el backoff satura en 5 s. | Un peer local puede provocar reconexiones o indisponibilidad; el recorrido API-only debe conservarse sin ocultar la degradación. |
| Parser ambiguo o smuggling de records | Un único framing `uint32` big-endian + JSON UTF-8 1..512 se aplica a stdin/stdout/TCP. El parser incremental drena chunks arbitrarios con máximo simultáneo de 516 bytes: libera la referencia interna y transfiere el payload sin copia antes del callback. Longitud cero/513+, truncado, UTF-8 inválido, BOM, JSON no objeto, duplicados, trailing, extra/missing y schemas desconocidos se rechazan. | Las referencias son test-only; el runtime futuro deberá demostrar paridad en Rust y TypeScript sin aceptar diferencias de parser. |
