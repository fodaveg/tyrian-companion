...
| Falso positivo de skin/mini no desbloqueada | Solo se emite con catálogo normalizado y cobertura completa simultánea de skins y minis; permiso ausente, 429, parcial o error fallan cerrados. Una skin aplicada no se confunde con `details.skins[]`. | ArenaNet puede cambiar disponibilidad o semántica; la señal siempre exige revisión manual. |
| Duplicación de alertas | Observación idempotente, dedupe durable por episodio+objeto y un notice agregado por poll; el reconocimiento también es durable. | Episodios distintos pueden alertar de nuevo deliberadamente. |
