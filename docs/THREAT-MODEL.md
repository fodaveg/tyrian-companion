...
- No hay endpoints del proyecto, subida de diagnósticos, analítica remota ni tráfico Mumble. H8.4
  cierra el futuro canal a TCP IPv4 `127.0.0.1` y records framed, pero no autoriza ni implementa
  red/IPC. Cualquier runtime posterior exige una nueva revisión sobre su diff y QA real.
...
| Contrato H8.1 Mumble v2 | Constantes públicas de versión, límites, fuentes, map id 866 y tipos TS | H8.5 implementa el helper/servidor aislado; el cliente/plugin no existe. El contrato exige retención `none` para raw y frames. |
| Contrato H8.4 IPC local | Esquemas, framing, token/nonce, secuencia, cadencia exclusiva sample/heartbeat, calentamiento, plazos y errores declarativos | No genera secretos, abre sockets ni procesa records. Los valores sintéticos, validators, framer, cadencia y fake clock viven solo en tests. |
...
| Helper Mumble no revisado o fuera de censo | Scanner v5 permite exactamente el contrato TS y los seis módulos Rust H8.5. El guard positivo censa la raíz nativa completa, dependencias/versiones, toolchain, tests y única isla Win32 `unsafe`; cualquier otro helper o output nativo falla. | Un nombre deliberadamente oculto o dependencia nativa indirecta todavía requiere review humana y análisis de artefactos de release. No hay aislamiento frente a código local malicioso. |
...
| Cadena de suministro y paquete separado | Cargo.lock fija checksums; el verifier rechaza licencia/source/proc-macro/custom-build desconocidos y valida un staging sintético exacto de cinco ficheros. CI inspecciona PE x64, CRT estático y dos builds, pero solo sube `UNSIGNED-NOT-FOR-RELEASE` un día. El ZIP del plugin sigue exactamente en tres ficheros. | No existe paquete productivo ni firma; la reproducibilidad de CI debe ser confirmada por el run real y un ZIP declarativo no sustituye análisis de bytes. |
...
| Parser ambiguo o smuggling de records | Un único framing `uint32` big-endian + JSON UTF-8 1..512 se aplica a stdin/stdout/TCP. La referencia H8.4 drena chunks arbitrarios con máximo simultáneo de 516 bytes y transfiere ownership antes del callback. H8.5 aplica la misma frontera Rust y rechaza longitud cero/513+, truncado, UTF-8, BOM, trailing, extra/missing y duplicados incluso con clave escapada; los buffers de token se invalidan tras parse/auth. | El cliente TypeScript aún no existe; deberá demostrar paridad sin aceptar diferencias de parser. |
...
- `npm run security:scan` ejecuta el scanner v5 sin red sobre el corpus Git relevante; el empaquetador invoca además su entrada cerrada sobre `manifest.json`, `main.js` y `styles.css` ya staged.
