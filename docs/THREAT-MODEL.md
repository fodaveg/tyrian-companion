...
| Comparación de botín presentada como aperturas | Solo un cierre Halloween finalizado, comparable y confirmado con `open=true` exclusivo entra en la comparación. La UI y el record usan `bagsDisappearedNet`; los 18 outcomes, ceros y razones de exclusión quedan atómicos con el sello y no se reconstruyen desde Markdown. BigInt evita overflow y reproduce la criba. | El neto no demuestra aperturas ni causalidad de cada ganancia; actividad externa no declarada puede sesgar la muestra. El modelo comunitario puede quedar obsoleto. |
| Aviso de precio duplicado o basado en historia incompleta | H11.5 solo recibe el puerto local después de captura/compactación H9.1. Exige hoy más los 30 días UTC completos previos, usa p90 nearest-rank y falla cerrado ante huecos/bid null/store inválido. La transición below→high, límite diario, cooldown y rearmado se deciden en una transacción multiwindow. | Es una serie local, no historia oficial ni garantía de ejecución al precio bid. Cerrar Obsidian crea huecos que bloquean el aviso hasta recuperar 30 días completos. |
