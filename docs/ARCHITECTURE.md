...
H3.9 añade `SessionContaminationReview` como dato derivado y verificable, no como texto libre. **Revisión humana retirada el 9 sep 2026** (David: «Tengo que confiar en el plugin... No tengo que aprobar nada»): `createSessionContaminationReview` (`src/sessions/session-contamination-review.ts`) ya no pregunta nada; fija `declaration: { status: 'absent' }` y unas `answers` constantes (`AUTOMATIC_ANSWERS`, `certainty: 'unsure'`, las diez actividades en `false`), que sobreviven solo porque `halloween-loot-comparison.ts` sigue leyendo `answers.certainty` para la elegibilidad de la estadística de la Bolsa de truco o trato (fuera de este lote). Reconstruye `BoundaryEvidence` desde los snapshots completos, llama al clasificador H2.7 con fronteras manuales, TP explícitamente `unavailable` y esa declaración fija, y rechaza cualquier resultado inválido. Como ninguna declaración humana puede contaminar ya una sesión, `status: 'contaminated'` y `activity_declared` quedan en el vocabulario solo para que un registro anterior al 9 sep se siga leyendo (ver `contamination.ts`, comentario junto a `classifyValidatedSessionDelta`). La revisión se calcula y persiste en la misma llamada que finaliza la sesión (`finalizeStoppedSessionInternal` → `stopInternal`, `manual-session-start-service.ts`); no hay ventana `provisional` editable ni modal que la muestre antes de guardar.
...
H11.3 solo sella una comparación cuando `reviewContamination` devuelve `finalized`, la sesión está marcada Halloween, el delta es `comparable`, la certeza es `confirmed`, `open=true` y todas las demás actividades son falsas, y el cambio neto de `36038` es negativo (`ineligibleReason`, `src/halloween/halloween-loot-comparison.ts:196-206`). **Divergencia medida (24 sep 2026):** desde que la revisión humana se retiró el 9 sep (ver H3.9 arriba), `review.answers` para toda sesión nueva es la constante `AUTOMATIC_ANSWERS`: `certainty: 'unsure'` y `activities.open: false`. Las dos condiciones de `ineligibleReason` (`review_not_confirmed` y `activities_not_open_only`) fallan siempre para esas sesiones, así que `bagsDisappearedNet` ya no puede sellarse para ninguna sesión finalizada después del 9 sep; el gate solo sigue satisfaciéndose leyendo un registro `SessionContaminationReview` guardado antes de esa fecha. No se ha tocado el código de este módulo en este lote (es documentación); el hallazgo se reporta aparte. La cantidad se llama `bagsDisappearedNet`: describe bolsas desaparecidas del inventario y nunca demuestra aperturas. Cada episodio conserva en la misma transacción que su sello final una fila elegible o ignorada, su razón y los 18 outcomes del modelo curado de 106.264 bolsas, incluidos los observados como cero. El record v2 persiste `modelId` y `modelVersion`; la validación resuelve un registry histórico inmutable por versión, de modo que añadir un modelo futuro no reinterpreta ni corrompe records antiguos. El formato legacy v1 queda fijado al modelo original y un id/versión desconocidos fallan cerrados. No se reconstruye esta evidencia desde notas.
