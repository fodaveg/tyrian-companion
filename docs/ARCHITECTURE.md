...
`HalloweenRuntime` es opt-in y su construcción no abre IndexedDB, Vault, red ni timers. Al activarse y conocer la referencia de cuenta, usa el mismo inspector durable que el historial para exigir schema exacto, escalares canónicos y hashes válidos antes de aceptar `tc_event: halloween`: schema v3 reconstruye la unión positiva exacta; v2 Halloween queda `partial` sin inventar ids y una v3 dañada detiene el aprendizaje. El escaneo se repite idempotentemente en cada activación y solo los eventos create/modify/delete de Markdown bajo la carpeta de sesiones vigente solicitan un refresh coalescido, sin polling ni timers; el fingerprint de evidencia permite incorporar notas sincronizadas o modificadas sin reabrir observaciones idénticas. Un meta record por vault/cuenta conserva la cobertura de aprendizaje, por lo que recargar o volver a una cuenta no reinicia arbitrariamente `first_seen`. `tyrian-companion-halloween` v3 separa observaciones, objetos vistos, pertenencia first-seen por episodio, sellos terminales, notices y meta; el ámbito usa hash de vault y referencia seudónima de cuenta, nunca `accountId`, snapshot completo o clave API. Corrupción, cuota, versión futura y `versionchange` fallan cerrados. Tras persistir, el mismo conjunto seen puede alimentar H9.1 por un puerto de ids; con H9.1 apagado no abre su base y, al activarse o incorporar notas nuevas, se reproducen como máximo 400 ids recientes.

El modo vivo solo acepta deltas cuando la sesión está ligada a una propuesta Halloween canónica aceptada; idle, actividad ordinaria y sesiones manuales no contaminan seen ni H9. Polling y cierre comparten la autoridad `session:<sessionId>` y una cola por episodio: el cierre espera vuelos anteriores, sella el episodio y ningún poll tardío puede persistir o encolar después. El cierre revisado conserva la pertenencia causal `first_seen` y reemplaza en una transacción cantidades, cobertura y razones provisionales, incluso eliminándolas si dejan de aplicar. Un final sin provisional emite un único `Notice`; reemplazar uno ya anunciado no repite el foreground, conserva `acknowledgedAt` y repetir un final idéntico es no-op.
