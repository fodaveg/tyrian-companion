...
`HalloweenRuntime` es opt-in y su construcción no abre IndexedDB, Vault, red ni timers. Al activarse y conocer la referencia de cuenta, escanea una sola vez las notas con `tc_event: halloween` antes de habilitar el modo vivo: schema v3 reconstruye la unión positiva exacta; v1/v2 o evidencia dañada quedan `partial` sin inventar ids. Un meta record por vault/cuenta conserva que ese aprendizaje terminó, por lo que recargar o volver a una cuenta no reinicia arbitrariamente `first_seen`. `tyrian-companion-halloween` v2 separa observaciones, objetos vistos, episodios, notices y meta; el ámbito usa hash de vault y referencia seudónima de cuenta, nunca `accountId`, snapshot completo o clave API. Corrupción, cuota, versión futura y `versionchange` fallan cerrados. Tras persistir, el mismo conjunto seen puede alimentar H9.1 por un puerto de ids; con H9.1 apagado no abre su base y, al activarse, se reproducen como máximo 400 ids recientes.

El modo vivo solo acepta deltas cuando la sesión está ligada a una propuesta Halloween canónica aceptada; idle, actividad ordinaria y sesiones manuales no contaminan seen ni H9. Polling y cierre comparten la autoridad `session:<sessionId>`. El cierre revisado reemplaza en una transacción las cantidades, cobertura y razones provisionales del episodio, incluso eliminándolas si dejan de aplicar, y nunca dispara un segundo `Notice`. Repetir el cierre es idempotente.

La política pura agrega motivos por objeto: valor neto demostrado por unidad sobre el umbral, Rare+ con `no_quote` demostrado o vinculado, primera aparición tras aprendizaje y skin/mini aún no desbloqueada. Precio distingue `quote|no_quote|unavailable|invalid|rate_limited`: omisión válida o `whitelisted:false` demuestra ausencia; 429, offline, respuesta o batch inválido degradan a partial/backoff y jamás se convierten en «sin cotización». El normalizador de catálogo v3 conserva `details.skins[]` positivo, único y ordenado y el `details.minipetId` ya existente; son ids que el objeto puede desbloquear, no skins aplicadas. Las señales de unlock requieren cobertura `complete` de `/v2/account/skins` y `/v2/account/minis` con permiso opcional `unlocks`; missing/partial/error no significa «bloqueado», y una rareza futura tampoco se promueve. El outbox conserva no leídos y exige reconocimiento explícito. `Notice` existe solo en el adaptador foreground.
