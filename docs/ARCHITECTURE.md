...
halloween runtime -> positive deltas -> dedicated IndexedDB -> durable alert inbox
	              -> public catalog/prices + optional authenticated unlock evidence
...
Cada refresh abre un único span propietario `session_projection`; una lectura fallida conserva
`storage_failure`, el `TypeError` histórico de precondición queda `precondition_failed` y otro bug de
proyección queda `internal_failure`. El cache no inventa un terminal de persistencia para el callback
...
Los resultados forman una unión cerrada: éxito, offline, rate limit, fallo transitorio o fatal. `apiPollOutcomeFromError` traduce únicamente `HttpTransportError` saneado: network/timeout y `500|502|503|504` son transitorios, `429` conserva `retryAfterMs`, y autenticación, respuestas no recuperables o errores desconocidos fallan cerrados. En captura account-wide, observar uno de esos fallos parciales detiene las pasadas restantes, conserva `passCoverages` incompleta y no publica snapshot; el scheduler recibe el fallo transitorio y agenda un único backoff, sin un segundo fan-out interno. Los fallos transitorios usan backoff exponencial acotado con jitter; un éxito reinicia el contador. La detección de sleep contrasta deadline de reloj de pared y monotónico, descarta la ejecución tardía y programa una sola reanudación fresca.

## Caché y cuota reales de la API de GW2

La API de cuenta sirve desde caché, así que consultarla más a menudo no acerca el dato al juego. Lawton Campbell, desarrollador de la API en ArenaNet, lo describe como *"the cache time is 5-10 minutes (nested caches)"*: son dos capas, el servidor de mapa persiste el personaje periódicamente y encima hay una caché HTTP. La cita procede de una declaración pública suya; este documento no registra su URL.

Cabeceras medidas el 2026-09-01: `/v2/build` sirve `max-age=3600`, `/v2/commerce/prices` sirve `max-age=120` y `/v2/commerce/listings` sirve `max-age=1200`. En esas mismas respuestas, `x-rate-limit-limit` declara 600 peticiones por minuto, no las 300 que documenta la wiki. Son valores observados ese día, no un contrato de ArenaNet: se remiden leyendo las cabeceras, no reescribiendo este párrafo.

La consecuencia de diseño gobierna al scheduler y a lo que el producto puede prometer. Consultar con más frecuencia que el `max-age` del endpoint solo gasta cuota releyendo bytes idénticos, por lo que la cadencia por defecto se elige contra esa caché, no contra la impaciencia del usuario. Y la frontera de una sesión detectada por API nunca será más precisa que un clic del usuario: la incertidumbre no viene de nuestro muestreo, sino de la persistencia del servidor de mapa y de la caché encima.

Hipótesis no medida, escrita aquí para que nadie la ascienda a hecho por repetición: que la cartera se actualice antes que las bolsas. El argumento estructural es plausible, porque la cartera es dato de cuenta mientras que el inventario de personaje arrastra además la persistencia del servidor de mapa, pero nadie lo ha medido. Ninguna decisión de producto puede apoyarse en ella hasta que exista la medición.
...
Mumble Link no forma parte del grafo de dependencias de v1. H8.5 implementa el helper nativo Mumble
Link en `native/mumble-helper`, cuya única salida es el binario `tyrian-mumble-helper.exe`; su
heartbeat expone entre otros el estado `warming_up` mientras espera la primera lectura válida tras
arrancar. El helper sigue sin firma Authenticode y sin QA real en ninguna plataforma, y toda la
frontera H8 permanece aislada, sin wiring desde `main` y con 0 bytes en el bundle publicado. El
relato completo de diseño de H8.1 a H8.7 (spike de validación, protocolo IPC, runtime del helper,
cliente TypeScript y frontera de lanzamiento seguro) se movió el 2026-09-10 a [histórico H8/Mumble
Link](historico/ARCHITECTURE-h8-mumble.md); las decisiones formales y sus triggers de reapertura
siguen vigentes en `docs/adr/0001` a `docs/adr/0005`.
...
`AssistedDetectionService` compone H3.5–H3.8 sin automatizar H3.1. Nace desarmado y no persiste el armado: cada recarga exige una acción nueva. `arm()` deduplica el doble clic, captura primero un snapshot estable y solo después programa polling. Cada tick captura otro snapshot, calcula H2.6 y entrega la evidencia al detector de inicio cuando la sesión está `idle` o al detector de inactividad cuando está `active`. Cambiar entre `idle` y una sesión concreta fuerza una nueva frontera antes de interpretar deltas, por lo que nunca se arrastra evidencia previa al inicio. Una propuesta pausa el scheduler y queda permanentemente visible en la vista hasta que el usuario inicia/detiene o descarta. Desarmar invalida capturas en vuelo, borra evidencia temporal y detiene timers; cambiar la referencia de credencial también desarma para impedir mezclar cuentas. Offline y wake se delegan al scheduler. La regla de Halloween es versionada y usa los ids públicos oficiales; nunca compara nombres localizados. `halloween.labyrinth-drops` v3 vigila los cinco drops del Laberinto (`36038`, `36041`, `36059`, `36060`, `36061`) pero **no los trata como intercambiables**: `36038` es el ancla (`anchorItemId`), su subida es necesaria para proponer inicio y su bajada dentro del mismo delta invalida la muestra con razón `anchor_decreased`, porque una bolsa que baja es una bolsa que se abre, no una que se farmea. Los otros cuatro siguen contando como ganancia, que es lo que hace alcanzable el criterio bajo la caché, pero ya no sustituyen a la bolsa. La v2 sí los trataba como intercambiables; la v1 se llamaba `halloween.trick-or-treat-bag` y vigilaba solo `36038`. El detector de inicio no exige dos consultas consecutivas con ganancia —la caché de 5-10 minutos de la API haría inalcanzable ese criterio a cadencias rápidas—, sino dos ganancias dentro de una ventana de evidencia contigua de `max(3 muestras, 30 minutos)`. La lista viva es `HALLOWEEN_RELEVANT_ITEM_RULE_SET` en `src/sessions/assisted-detection-service.ts`.

H3.9 añade `SessionContaminationReview` como dato derivado y verificable, no como texto libre. **Revisión humana retirada el 9 sep 2026** (David: «Tengo que confiar en el plugin... No tengo que aprobar nada»): `createSessionContaminationReview` (`src/sessions/session-contamination-review.ts`) ya no pregunta nada; fija `declaration: { status: 'absent' }` y unas `answers` constantes (`AUTOMATIC_ANSWERS`, `certainty: 'unsure'`, las diez actividades en `false`). Esas `answers` siguen persistiéndose (forma cerrada que `isSessionContaminationReview`/`isSessionContaminationReviewShape` validan), pero desde H18.32 (26 sep 2026) ningún código de producción las lee: `halloween-loot-comparison.ts` dejó de usar `answers.certainty`/`activities` como puerta de elegibilidad — ver más abajo. Reconstruye `BoundaryEvidence` desde los snapshots completos, llama al clasificador H2.7 con fronteras manuales, TP explícitamente `unavailable` y esa declaración fija, y rechaza cualquier resultado inválido. Como ninguna declaración humana puede contaminar ya una sesión, `status: 'contaminated'` y `activity_declared` quedan en el vocabulario solo para que un registro anterior al 9 sep se siga leyendo (ver `contamination.ts`, comentario junto a `classifyValidatedSessionDelta`); ese mismo `classification.status` es justo lo que H18.32 empezó a leer para la comparación de botín. La revisión se calcula y persiste en la misma llamada que finaliza la sesión (`finalizeStoppedSessionInternal` → `stopInternal`, `manual-session-start-service.ts`); no hay ventana `provisional` editable ni modal que la muestre antes de guardar.
...
H7.13 añade un journal de piloto separado y opt-in, `tyrian-companion-pilot-metrics:<vaultId>`, que se abre de forma lazy y nunca es requisito para presentar, aceptar, descartar, recuperar o cerrar una sesión. El perfil local fija plataforma y estratos de versión; las propuestas conservan la ventana y el intervalo vigentes al generarse. Materializar una tarjeta de propuesta puede disparar únicamente su escritura local fail-open `review_presented`: es la excepción explícita necesaria para medir cobertura, sin red, lectura/escritura Vault ni retraso o deshabilitado de controles. Aceptar/iniciar/parar nunca intercala un modal de instrumentación: la acción de producto continúa con corrección temporal `null` cuando no existe una adjudicación humana explícita. Toda propuesta presentada termina por decisión humana o por una exclusión operativa cerrada `expired|superseded|invalidated`: estas exclusiones se publican aparte y no contaminan `k/n` ni cobertura. `accepted_workflow_failed` sella el primer intento fallido; un éxito, reintento o cierre operativo posterior es un no-op idempotente y no puede reinterpretar ni corromper esa observación. Las recoveries pueden permanecer sin clasificar; esa ausencia conserva la evidencia y fuerza un veredicto inconcluso, pero no bloquea Recovery. Una clasificación persistida se rehidrata al recargar y su selector queda cerrado para impedir una adjudicación contradictoria.

El store mantiene `sampleRevision`, un entero monotónico guardado en el store de perfil. Cada mutación real de perfil o evidencia lo incrementa e invalida la revisión en la misma transacción; duplicados y no-ops no lo alteran. Desactivar borra perfil, evidencia y revisión, pero incrementa y retiene únicamente este contador no personal para que una reactivación no pueda reutilizar una generación antigua. Guardar una revisión compara la revisión esperada dentro de una transacción que se serializa con todas las mutaciones y devuelve `stale` sin degradar la salud si otra ventana cambió la muestra. Las revisiones legacy se leen, pero nunca se confían. Estadísticas y export exigen coincidencia exacta de `sampleRevision` y entorno.

La agregación H7.13 no mezcla plataformas: publica recuentos, cobertura, Wilson 95 %, mediana/p90/máximo de precisión, recoveries, sesiones completadas y `pass|fail|inconclusive` por plataforma, además de estratos diagnósticos por versiones. Desde Ajustes, una acción explícita muestra primero la preview y después crea, sin sobrescribir, cuatro ficheros JSON/CSV deterministas bajo `exports/`; no hay envío ni telemetría remota. Las referencias de propuesta son hashes SHA-256 con separación de dominio: seudónimos, no anonimización. **Limpiar métricas** borra observaciones y revisión conservando el perfil; **Desactivar y borrar** elimina perfil, revisión y observaciones, conserva solo el contador generacional no personal y deja los hooks fail-open. Ninguna acción borra exports ya creados.
...
El guard arquitectónico Vitest censa todos los módulos productivos `*recommendation*.ts` y falla ante imports de cliente/operación/HTTP/transporte/request/store/secret/Obsidian, campos de capacidad, símbolos públicos de ejecución y llamadas ordinarias a fetch/request/requestUrl/execute o métodos de orden. El helper compartido extrae specifiers desde el AST de TypeScript y cubre imports y exports estáticos, imports laterales, `import = require`, tipos `import()`, `import()` dinámico y `require()` cuando el argumento es literal. Se saboteó con un import real de `GuildWars2Client`: el primer censo incompleto quedó verde, se amplió para incluir `recommendation-envelope.ts`, el mismo sabotaje pasó a rojo y después se restauró con hash y modo idénticos. Las sintaxis admitidas tienen regresión contra el mismo parser/clasificador exportado por el guard. Su límite explícito es el análisis estático: no pretende resolver specifiers computados u ofuscados, acceso dinámico ofuscado ni código fuera de la frontera nominal.
...
propagan la misma evidencia. Missing, partial, stale, future o exhausted producen revisión/bloqueo;
listing conserva solo el mejor ask como referencia estimada y manual, nunca como comprador ni total
realizable.
...
`TyrianSettings.apiKeySecret` guarda únicamente el nombre seleccionado por `SecretComponent`. `ObsidianApiKeyProvider` comprueba con `listSecrets` que la referencia siga existiendo, por lo que borrar el secreto impide la conexión. `GuildWars2Client.beginOperation()` lee el valor una única vez y devuelve una operación que reutiliza esa copia efímera en `tokeninfo`, `account` y sus reintentos. Nunca se persiste. Cambiar o quitar la referencia publica el nuevo ajuste y llama a `ConnectionService.reset()` solo después de que el guardado durable se confirme; un rechazo conserva exactamente la referencia y el estado anteriores.
...
`classifySessionDelta(delta, context)` acepta ambos argumentos como datos no confiables y aplica guards estructurales estrictos antes de acceder a ellos: identidad, ventanas, superficies, razones, warnings, cambios, composición, evidencia TP, frontera y cada variante de declaración. `null`, campos anidados ausentes, variantes desconocidas, propiedades extra incoherentes o aritmética corrupta devuelven `invalid` sin lanzar. Una composición solo es válida si ambos lados son no vacíos, distintos y canónicamente ordenados, conservan suma por id dentro de enteros seguros y respetan estado/ubicación: bolsa equipada solo en `equipped_bag`, root pending solo en delivery e hijos unitarios con relación embebida válida fuera de una bolsa equipada. Las composiciones de divisa conservan igualmente el total. Después valida que la frontera coincida con ids, cuenta, ventana y superficies del delta. El contexto aporta evidencia TP `complete|partial|unavailable` con eventos normalizados dentro de ventana, declaración explícita del usuario y certeza de fronteras. La prioridad es `invalid > contaminated > estimated > exact`: cambios de delivery con cobertura completa, compra/venta TP, descenso observado de una divisa monetaria, roster cambiado o actividades declaradas contaminan. H13.6 acota qué cuenta como descenso monetario: solo Coin (`1`) y Gem (`4`), porque son las que compran; toda otra divisa que baje (llaves, viales, magia volátil, karma) es un insumo de farmeo y deja `consumable_currency_spent`, que estima. Por el mismo motivo una apertura declarada deja `open_activity_declared` y una pérdida neta de objetos deja `item_losses_observed`: las tres piden `review_consumed_inputs` y son las que convierten el rendimiento en banda, salvo que la pérdida sea enteramente entrada de farmeo (lista curada o `farmedLossItemIds`, H14.1), en cuyo caso `item_losses_observed` no degrada y desde H18.32 (26 sep 2026) lleva `detail: 'exempt'` — la señal que el gate de elegibilidad de la comparación de botín lee para distinguir esa pérdida de una real. Datos residuales bajo cobertura ausente/asimétrica no se interpretan como actividad. Un aumento de wallet es ambiguo mientras falte confirmación, pero una declaración limpia con fronteras manuales puede resolverlo dejando razón informativa. La evidencia observada siempre domina una declaración limpia en conflicto.
...
H3.3 amplía el mismo orquestador con una acción explícita de parada. `request_stop` fija una sola frontera temporal y mantiene el heartbeat; después una nueva operación con clave efímera captura el snapshot final A/B/C. Antes de capturar ese snapshot, `stopInternal` consulta `settlementWait` (`src/sessions/session-api-settlement.ts`): sin `force`, si no han pasado `API_SETTLEMENT_WINDOW_MS` (10 minutos) desde `request_stop`, devuelve `awaiting_settlement` en vez de capturar. Esa espera existe porque la API de cuenta de Guild Wars 2 no sirve en vivo: el servidor de mapa persiste el personaje periódicamente y encima hay una caché HTTP de 5 a 10 minutos (misma cita de Lawton Campbell que arriba); capturar el final al instante subcuenta el botín de los últimos minutos. Pasada una hora (`API_SETTLEMENT_STALE_AFTER_MS`) la captura ya no espera más, pero la sesión se marca `exceeded` en vez de `settled` y su clasificación se degrada. El workflow exige calidad estable, calcula `compareStorageSnapshots(baseline, final)` y rechaza resultados `invalid`. Justo antes de `confirm_stop` vuelve a ejecutar `assertOwned` con el handle renovado. Solo entonces publica `provisional` y conserva el delta físico en memoria para la UI y la revisión H3.9. Un fallo transitorio de captura o delta deja `stopping`, el baseline original y la misma frontera intactos, por lo que **Retry stop** no recaptura ni pierde el inicio. Una pérdida de fence o coordinación pasa a `error` con el estado `stopping` completo. H3.9 sigue siendo dueño de revisión, clasificación aceptada, finalización y persistencia; H3.3 no llama todavía al clasificador H2.7.
...
H18.12 separa ese record y su lease por vault. IndexedDB es por origen y todas las ventanas de Obsidian comparten `app://obsidian.md`, así que dos vaults con el plugin leían el mismo record y competían por el mismo lease. Cada vault usa ahora `tyrian-companion-session-runtime:<vaultId>` y `tyrian-companion-coordination:<vaultId>`, con el mismo `vaultId` (SHA-256 de la ruta del vault) que ya reciben el resto de stores del vault. `SessionStorageScope` decide la pareja una vez por ejecución, de forma lazy, y la comparten el store y el coordinador. La pareja sin vault que escribió una versión anterior no se migra ni se borra: la adopta tal cual, con su nombre de siempre, el primer vault que carga esta versión mientras esa pareja aún guarda un record y que nunca abrió su pareja propia. La adopción deja una marca `legacy-vault-owner` en la base antigua, dentro de la misma transacción que la comprueba, para que sobreviva a reinicios y dos vaults simultáneos no puedan ganar ambos. Los demás vaults no vuelven a leerla. Una base antigua sin sesión no la adopta nadie, y una marca ilegible cuenta como de otro. Record y lease se adoptan siempre juntos: recuperar exige la misma identidad de máquina con la que se cercó el record.

Un cambio de la clave de API a otra cuenta a mitad de sesión hace que la captura final sea de otra cuenta; el delta sale `invalid` con `account_mismatch`. Eso ya no es `delta_invalid`, que se reintentaba cada cinco minutos para siempre, sino `account_changed`: la sesión sigue `stopping` con su evidencia intacta, el fallo queda declarado en la tarjeta y no se reintenta solo. Solo el reintento visible, o la siguiente carga del plugin, vuelve a intentarlo, y termina con normalidad si la clave vuelve a ser de la cuenta original.

`exact` exige delta completo/comparable, fronteras confirmadas manualmente y ausencia de señales de actividad externa; ya no exige ninguna declaración humana (revisión retirada el 9 sep, ver H3.9 arriba), porque `declaration` llega siempre `{ status: 'absent' }`. La salida v1 usa scope `observed_storage_net`, razones deduplicadas/canónicas, confianza y permisos explícitos; `reviewRequests` está siempre vacío. Un resultado contaminado (solo alcanzable hoy al leer un registro anterior al 9 sep) puede finalizar y mostrar solo el neto; uno estimado permite valoración provisional pero no rendimiento bruto y finaliza sin pedir nada a nadie; uno inválido bloquea todo. Las recomendaciones permanecen deshabilitadas incluso en exacto hasta que existan motores económicos. H3.9 conserva ownership de la derivación automática, finalización y persistencia; ya no posee ninguna pregunta.

## Ajustes y migración

El esquema actual lo fija `SETTINGS_SCHEMA_VERSION` en `src/core/settings.ts`, hoy `14`; ese es el valor a comprobar, no este párrafo. `migrateSettings` convierte de forma idempotente los datos anteriores, descarta propiedades desconocidas y valida enums e intervalos. La migración v2→v3 añadió `managedAssetsRoot:null`; v4 separa los destinos portables de `legacyOutputFolder`/`legacyManagedAssetsRoot`; v5 añade el histórico de precios desactivado con intervalo 15 minutos, raw 7 días y diario 180 días; v6 añade Halloween desactivado y su umbral de 10.000 cobre; v7 añade el aviso local de precio desactivado, margen 0 puntos básicos y cooldown 24 h; v8 añade `halloweenPersonalValuation:{version:1,values:[]}` sin valores predefinidos; v9 añade la capacidad opcional por material account-wide (H9.17); v10 añade kit de reciclaje, estrategia de venta y, opcionalmente, segundos por objeto y coste de oportunidad por hora (H9.3); v11 añade el registro de diagnóstico local con sus dos campos `debugLoggingEnabled`/`debugLoggingLevel` (H6.17); v12 es una migración de beta que fuerza una sola vez la cadencia de polling por defecto al subir desde un esquema anterior y respeta después cualquier edición explícita ya guardada como v12; v13 añade `recommendationCapitalThresholdCopper` (10 oros / 100.000 cobre por defecto), el umbral de capital parado que dispara la regla (c) de `docs/SPEC-recomendacion-por-objeto.md`, deliberadamente distinto de `valuableLootThresholdCopper`; v14 añade `legendaryTargetItemIds` (M4, vacía por defecto), la lista de legendarias objetivo que gobierna la regla (a) de esa misma spec. Solo v11 y v12 se aceptan como portadoras de los campos de diagnóstico, mediante `hasExplicitDebugSettings`; cualquier otro esquema recibe el default.

Los valores por defecto de una instalación nueva viven en `DEFAULT_SETTINGS` del mismo fichero: el idioma se resuelve desde Obsidian con `getLanguage()` y reserva `en` en vez de forzar español, el registro de diagnóstico nace apagado y en nivel `warn`, y el intervalo de consulta es de 10 minutos. **Esos defaults no alcanzan a una instalación existente.** En `data.json` un valor heredado del default es indistinguible de uno elegido por el usuario, así que la migración no puede corregir uno sin pisar el otro: un vault que ya tenía el registro encendido lo conserva hasta que su dueño lo apague en Ajustes. La reescritura canónica de ajustes conserva solo esos campos legacy autorizados y elimina cualquier propiedad desconocida. Si un valor relativo antes aceptado deja de ser portable (nombre reservado o longitud) queda retenido read-only como legacy, no puede producir notas ni assets nuevos ni alterar el puntero durable. Move/Remove siempre inspeccionan la raíz esperada aunque el puntero ya la nombre: el manifiesto debe ser owned y exacto, y Move exige además `ready` antes de aplicar destino; un puntero que nombra otra raíz es conflicto. Si Remove ya dejó ese manifiesto exacto detached y se perdió su respuesta, el retry solo confirma ese terminal con puntero inicial vacío, sin adoptar ni tocar Vault, y vuelve a leer el puntero para exigir estado, generación y raíz idénticos tras la inspección. Con un puntero aún en la raíz esperada, Remove retoma su transición normal y solo entonces lo libera. La autoridad para carreras continúa exclusivamente en el puntero IndexedDB: la migración nunca lo modifica. La carpeta de salida solo acepta segmentos relativos separados por `/`: normaliza a NFC en vez de rechazar NFD, y sigue rechazando navegación, controles y surrogates sin emparejar, barras inversas, `:*?"<>|`, punto o espacio final, nombres Windows reservados, rutas absolutas y el directorio de configuración real del vault. `resolveVaultFolderInput` valida un valor tecleado en Ajustes con el mismo contrato pero sin el fallback de `normalizeVaultFolder`: un rechazo legítimo se muestra junto al campo y nunca sustituye en silencio el valor guardado por el default.

`outputFolder` es la única raíz que el usuario elige: también gobierna dónde viven Bases y plantillas, no solo notas. `managedAssetsRoot` dejó de poder quedar divergido sin que el plugin reaccione. Un cambio explícito de carpeta con assets ya instalados dispara `reconcileManagedAssetsRoot`, que reubica Bases/plantillas al nuevo destino con la misma `ManagedAssetsLifecycle.move` journaled del botón manual "Mover" —nunca una copia paralela—, y dos raíces vuelven a quedar iguales tras un `updateSettings` con `outputFolder` cambiado. La misma reconciliación corre una vez al terminar `initializeRuntime`, sin bloquear el arranque, para curar una instalación que ya empezó divergida (una raíz de assets que nunca siguió un cambio de carpeta posterior a H5.8). Si Obsidian ya reserializó las cinco Bases en el destino y dejó el manifiesto v2 `ready` en el origen, solo la operación específica de relocation puede adoptar ese destino: exige autoridad origen exacta/current, conjunto destino completo sin extras y equivalencia semántica por cada asset; un install ordinario sigue rechazando esos ficheros markerless. La adopción escribe primero un journal `relocate`, conserva los bytes destino, cambia después el puntero y solo al final marca el origen como `detached`; un retry reanuda el mismo intent. Move se niega sobre cualquier faltante, extra, modificación o autoridad ajena, así que una reconciliación bloqueada deja ambas raíces byte-idénticas —nunca a medias— y sigue mostrando la divergencia y el botón "Mover" manual en Ajustes. Un `Notice` avisa del resultado (reubicado o bloqueado) porque mover ficheros del vault sin que el usuario lo vea al abrir Obsidian se consideró peor que avisar después del hecho. Una raíz legacy queda deliberadamente fuera de esta reconciliación automática: solo se adopta mediante el Move explícito ya documentado, nunca por sí sola al arrancar.
...
## Observación y alertas de Halloween H11.1/H11.2/H11.4

`HalloweenRuntime` es opt-in y su construcción no abre IndexedDB, Vault, red ni timers. Al activarse y conocer la referencia de cuenta, usa el mismo inspector durable que el historial para exigir schema exacto, escalares canónicos y hashes válidos antes de aceptar `tc_event: halloween`: schema v3 reconstruye la unión positiva exacta; v2 Halloween queda `partial` sin inventar ids y mantiene `first_seen` en aprendizaje, mientras una v3 dañada detiene el aprendizaje. El escaneo se repite idempotentemente en cada activación y solo los eventos create/modify/delete/rename de Markdown bajo la carpeta de sesiones vigente solicitan un refresh coalescido, sin polling ni timers; el fingerprint de evidencia permite incorporar notas sincronizadas o modificadas sin reabrir observaciones idénticas. El port de notas del wiring (`main.ts`) solo entrega ficheros bajo `${outputFolder}/sessions/`, nunca el vault completo (H14.11: 3.808 `vault.read` por disparo medidos sobre una bóveda real antes de este filtro), y un `HalloweenBackfillCache` por instancia de plugin memoiza el resultado de inspección por ruta+`mtime`, así que un disparo sin cambios no relee ni una nota. Re-resolver la misma cuenta ya conocida (p. ej. una segunda «Comprobar conexión») no vuelve a activar el runtime: solo un cambio de cuenta, la temporada abriéndose o un ajuste explícito reactivan. Una observación viva espera cualquier backfill activo antes de decidir `first_seen`. Un meta record por vault/cuenta conserva la cobertura de aprendizaje, por lo que recargar o volver a una cuenta no la completa arbitrariamente. `tyrian-companion-halloween` v5 separa observaciones, objetos vistos, pertenencia first-seen por episodio, sellos terminales, notices, comparaciones de botín, estado/avisos de precio y meta; el ámbito usa hash de vault y referencia seudónima de cuenta, nunca `accountId`, snapshot completo o clave API. Corrupción, cuota, versión futura y `versionchange` fallan cerrados. Tras persistir, el mismo conjunto seen puede alimentar H9.1 por un puerto de ids; con H9.1 apagado no abre su base y, al activarse o incorporar notas nuevas, se reproducen como máximo 400 ids recientes. Un backfill histórico nunca reduce `lastObservedAt`, así que tampoco degrada el orden del replay.

El modo vivo solo acepta deltas cuando la sesión está ligada a una propuesta Halloween canónica aceptada; idle, actividad ordinaria y sesiones manuales no contaminan seen ni H9. Polling y cierre comparten la autoridad `session:<sessionId>` y una cola por episodio: el cierre espera vuelos anteriores, incluso con cero ganancias sella el episodio, elimina cualquier provisional y ningún poll tardío puede persistir o encolar después. El sello durable conserva la observación final estable —no catálogo, precios ni unlocks remotos— y el primer cierre multiwindow gana; otra resolución del mismo delta es no-op, no corrupción. El cierre revisado conserva la pertenencia causal `first_seen` y reemplaza en una transacción cantidades, cobertura y razones provisionales, incluso eliminándolas si dejan de aplicar. Un final sin provisional emite un único `Notice`; un final equivalente o subconjunto ya reconocido conserva `acknowledgedAt`, mientras un item o motivo nuevo vuelve a unread y emite un único foreground solo si todos los provisionales previos estaban reconocidos. Repetir el cierre no vuelve a notificar.

La política pura agrega motivos por objeto: valor neto demostrado por unidad sobre el umbral, Rare+ con `no_quote` demostrado o vinculado, primera aparición tras aprendizaje y skin/mini aún no desbloqueada. Precio distingue `quote|no_quote|unavailable|invalid|rate_limited`: omisión válida o `whitelisted:false` demuestra ausencia; 429, offline, respuesta o batch inválido degradan a partial/backoff y jamás se convierten en «sin cotización». Catálogo proyecta su propia cobertura cerrada y un fallo nunca deja la UI en completa. El normalizador de catálogo v3 conserva `details.skins[]` positivo, único y ordenado y el `details.minipetId` ya existente; son ids que el objeto puede desbloquear, no skins aplicadas. Las señales de unlock exigen cobertura `complete` de su dimensión concreta (`/v2/account/skins` o `/v2/account/minis`) con permiso opcional `unlocks`: el fallo de minis no invalida evidencia completa de skins, ni al revés, y missing/error nunca significa «bloqueado». Una rareza futura tampoco se promueve. El outbox conserva no leídos y exige reconocimiento explícito. `Notice` existe solo en el adaptador foreground.

Las notas nuevas suben a schema v3 y añaden `tc_positive_item_deltas_json`, matriz canónica ordenada `[itemId,cantidad]`. La reconstrucción v3 es exacta. v1/v2 siguen legibles, pero cualquier backfill declara `partial` porque no contienen deltas machine-readable; nunca se infiere del Markdown humano. Settings v6 añade el toggle desactivado y un umbral por defecto de 10.000 cobre.

## Comparación de botín y aviso de precio H11.3/H11.5

**Hallazgo del 24 sep 2026, corregido el 26 sep (H18.32):** hasta entonces H11.3 sellaba una comparación solo cuando la revisión humana (retirada el 9 sep, ver H3.9 arriba) confirmaba `certainty: 'confirmed'` y `open=true` en exclusiva. Como `createSessionContaminationReview` fija esas respuestas a la constante `AUTOMATIC_ANSWERS` para toda sesión nueva (`certainty: 'unsure'`, `activities.open: false`), esa puerta fallaba siempre y `bagsDisappearedNet` no volvía a sellarse para ninguna sesión finalizada después del 9 sep.

H18.32 sustituyó esa puerta por la clasificación automática de la propia sesión (`SessionDeltaClassification`, H2.7): H11.3 sella una comparación cuando el delta es `comparable`, la clasificación no es `contaminated` ni `invalid` (`estimated` no descalifica por sí solo — abrir contenedores/consumir no degrada por debajo de `estimated`, regla firmada en `docs/PRODUCT.md`, «Decisiones de rumbo del 2026-09-08») y el cambio neto de `36038` es negativo (`ineligibleReason`, `src/halloween/halloween-loot-comparison.ts`).

**Corrección de revisión (26 sep 2026):** el primer cierre de H18.32 leía solo `status` y dejaba pasar una sesión `estimated` cuyo motivo SÍ mueve objetos — vender una bolsa en el bazar degrada a `estimated`/`tp_sell_observed`, nunca a `contaminated`, así que `bagsDisappearedNet` contaba las bolsas vendidas como abiertas. El gate ahora recorre también `reasons`: `tp_sell_observed`, `tp_buy_observed`, `delivery_items_changed`, `roster_changed`, `character_unobserved`, `delta_limited`, y `item_losses_observed` sin `detail: 'exempt'` (motivo nuevo `external_item_movement`) excluyen la sesión, porque cada uno es evidencia de que un objeto pudo moverse por algo distinto de abrir. `wallet_decreased`, `delivery_coins_changed`, `consumable_currency_spent`, `open_activity_declared` y las dos razones de ventana de la API (`api_settlement_window_skipped`/`exceeded`) nunca excluyen: ninguna puede mover un recuento de objetos. `contamination.ts` ahora marca `item_losses_observed` con `detail: 'exempt'` cuando TODAS las pérdidas del delta fueron entrada de farmeo (lista curada o `farmedLossItemIds`); su ausencia significa que alguna pérdida no lo fue.

Si la clasificación no se pudo resolver en el punto de llamada, la comparación queda no elegible con su propio motivo (`classification_unavailable`), nunca tratada como limpia por defecto. Los motivos retirados (`review_not_confirmed`, `activities_not_open_only`) siguen en el vocabulario solo para que un record v1/v2 persistido antes de H18.32 se siga leyendo y pintando; un record nuevo nunca los produce. La cantidad se llama `bagsDisappearedNet`: describe bolsas desaparecidas del inventario y nunca demuestra aperturas. Cada episodio conserva en la misma transacción que su sello final una fila elegible o ignorada, su razón y los 18 outcomes del modelo curado de 106.264 bolsas, incluidos los observados como cero. El record v2 persiste `modelId` y `modelVersion`; la validación resuelve un registry histórico inmutable por versión, de modo que añadir un modelo futuro no reinterpreta ni corrompe records antiguos. El formato legacy v1 queda fijado al modelo original y un id/versión desconocidos fallan cerrados. No se reconstruye esta evidencia desde notas.

La criba usa enteros `BigInt`: exige al menos 1.100 bolsas netas, expectativa `E>=20`, diferencia relativa mínima del 10% y `|z|>=3,45`, umbral de Bonferroni para 18 outcomes. Numerador y denominador esperados quedan persistidos para reproducir el cálculo exacto; la métrica Pearson global es solo descriptiva. El panel distingue todavía no finalizada, ignorada con razón, recopilando, suficiente sin desviación, desviación y fallo de store, y muestra una tabla con los 18 resultados.

H11.5 no crea cliente HTTP, scheduler ni timer. Después de una captura y compactación correctas, H9.1 ofrece al runtime un puerto local de lectura diaria de `36038`. El cierre de puja provisional de hoy se compara con el p90 nearest-rank, posición 27, de los 30 días UTC completos inmediatamente anteriores. Falta de un solo día, `bid:null`, dato corrupto, captura futura aunque pertenezca al mismo día UTC o cierre de hoy ausente produce `insufficient_history`; no se reutiliza el percentil H9.1 de 42 días ni se rellenan huecos. El runtime conserva `priceHistoryActive` como autoridad explícita y la incluye en cada cerca de generación/cuenta: si H9.1 se apaga, queda `disabled` y ni una activación o callback antiguos pueden reabrir la base, evaluar ni producir outbox.
...
La composición económica conserva byte por byte la explicación y decisión `liquidOnly`. Con overlay vacío o parcial muestra el ajuste conocido únicamente como límite inferior y mantiene `personalOpenEV` y decisión personal en `null`; la recomendación continúa fundamentada en EV líquido. Solo un overlay completo suma el ajuste al EV mediante aritmética exacta, aplica el mismo umbral de apertura del 10% y puede pasar a ser la recomendación principal. El disclosure de Trick-or-Treat Bag enseña siempre venta inmediata, EV líquido, umbral, ambas decisiones cuando difieren, fundamento, EV personal o pendientes y la exclusión agregada; no introduce intervalos estadísticos.
...
registran los nombres visibles del frontmatter con claves `properties.note.tc_*` salvo la columna de
actualización (`properties.file.mtime`), que es la forma canónica que Obsidian serializa. Las
...
valoración completa y calcula sacos/h y cobre inmediato/h desde sumas `BigInt` y duración total con
redondeo half-up. Una sesión no cualificada se cuenta como exclusión y nunca aporta una tasa parcial.
