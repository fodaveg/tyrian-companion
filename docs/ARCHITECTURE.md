...
`HalloweenRuntime` es opt-in y su construcción no abre IndexedDB, red ni timers. Solo recibe cambios positivos H2.6 desde polling asistido o cierre de sesión y los presenta como **cambio observado**, nunca como atribución exacta de drop. `tyrian-companion-halloween` v1 separa observaciones, objetos vistos, episodios deduplicados y notices; idempotencia y primera aparición se confirman en una transacción. El ámbito durable usa hash de vault y referencia seudónima de cuenta, nunca `accountId`, snapshot completo o clave API. Corrupción, cuota, versión futura y `versionchange` fallan cerrados. Tras persistir puede alimentar H9.1 por un puerto de ids; con H9.1 apagado no abre su base y, al activarse, se reproducen como máximo 400 ids recientes.

La política pura agrega motivos por objeto: valor neto demostrado por unidad sobre el umbral, Rare+ sin cotización o vinculado, primera aparición tras aprendizaje y skin/mini aún no desbloqueada. El normalizador de catálogo v3 conserva `details.skins[]` positivo, único y ordenado y el `details.minipetId` ya existente; son ids que el objeto puede desbloquear, no skins aplicadas. Las señales de unlock requieren cobertura `complete` de `/v2/account/skins` y `/v2/account/minis` con permiso opcional `unlocks`; missing/partial/error no significa «bloqueado», y una rareza futura tampoco se promueve. El outbox genera un notice agregado por observación, deduplica por episodio+objeto, conserva no leídos y exige reconocimiento explícito. `Notice` existe solo en el adaptador foreground.
