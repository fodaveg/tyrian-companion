...
Settings v8 persiste el overlay vacío por defecto, ordenado y sin duplicados, con independencia del toggle y de los avisos Halloween. El editor ES/EN es un componente separado de diez filas: vacío elimina, `0` guarda cero y un valor inválido conserva el último guardado con error inline y foco. Guardar consulta dinámicamente el overlay nuevo y reclasifica la última captura fresca retenida por el Inventory Advisor sin red ni nueva captura; si no existe captura en memoria, el cambio queda disponible para el siguiente Refresh explícito.
