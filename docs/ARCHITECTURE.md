...
H11.5 no crea cliente HTTP, scheduler ni timer. Después de una captura y compactación correctas, H9.1 ofrece al runtime un puerto local de lectura diaria de `36038`. El cierre de puja provisional de hoy se compara con el p90 nearest-rank, posición 27, de los 30 días UTC completos inmediatamente anteriores. Falta de un solo día, `bid:null`, dato corrupto, captura futura aunque pertenezca al mismo día UTC o cierre de hoy ausente produce `insufficient_history`; no se reutiliza el percentil H9.1 de 42 días ni se rellenan huecos. Si H9.1 está apagado, el aviso queda `disabled` y no lo activa.
