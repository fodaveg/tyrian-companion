...
H7.13 añade un journal de piloto separado y opt-in, `tyrian-companion-pilot-metrics:<vaultId>`, que se abre de forma lazy y nunca es requisito para presentar, aceptar, descartar, recuperar o cerrar una sesión. El perfil local fija plataforma y estratos de versión; las propuestas conservan la ventana y el intervalo vigentes al generarse. Materializar una tarjeta de propuesta puede disparar únicamente su escritura local fail-open `review_presented`: es la excepción explícita necesaria para medir cobertura, sin red, lectura/escritura Vault ni retraso o deshabilitado de controles. Toda propuesta presentada termina por decisión humana o por una exclusión operativa cerrada `expired|superseded|invalidated`: estas exclusiones se publican aparte y no contaminan `k/n` ni cobertura. Las recoveries pueden permanecer sin clasificar; esa ausencia conserva la evidencia y fuerza un veredicto inconcluso, pero no bloquea Recovery. El store serializa cada observación y su terminal, invalida atómicamente la revisión de pérdidas silenciosas ante cualquier mutación real o cambio de perfil y aplica un límite duro de 10.000 observaciones sin poda automática.

La agregación H7.13 no mezcla plataformas: publica recuentos, cobertura, Wilson 95 %, mediana/p90/máximo de precisión, recoveries, sesiones completadas y `pass|fail|inconclusive` por plataforma, además de estratos diagnósticos por versiones. La revisión de pérdidas solo aplica al entorno y a la muestra exacta que seguían vigentes al guardarla. Desde Ajustes, una acción explícita muestra primero la preview y después crea, sin sobrescribir, cuatro ficheros JSON/CSV deterministas bajo `exports/`; no hay envío ni telemetría remota. Las referencias de propuesta son hashes SHA-256 con separación de dominio: seudónimos, no anonimización. **Limpiar métricas** borra observaciones y revisión conservando el perfil; **Desactivar y borrar** elimina perfil, revisión y observaciones y deja los hooks fail-open. Ninguna acción borra exports ya creados.
